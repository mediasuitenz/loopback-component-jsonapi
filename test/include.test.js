...
describe('include option', function () {
  ...
  describe('including a specific method', function () {
    beforeEach(function () {
      JSONAPIComponent(app, {
        include: [{ methods: 'customMethod' }]
      })
    })

    it(
      'should apply jsonapi to post model output for customMethod method',
      function (done) {
        request(app).get('/posts/custom').expect(200).end(function (err, res) {
    ...
    it(
      'should apply jsonapi to comment model output for customMethod method',
      function (done) {
        request(app)
          .get('/comments/custom')
          .expect(200)
  ...
  describe('including a specific method on a specific model', function () {
    beforeEach(function () {
      JSONAPIComponent(app, {
        include: [{ model: 'post', methods: 'customMethod' }]
      })
    })
    ...
    it(
      'should not apply jsonapi to comment model output for customMethod method',
      function (done) {
        request(app)
          .get('/comments/custom')
          .expect(200)
  ...
  describe(
    'including a specific set of methods on a specific model',
    function () {
      beforeEach(function () {
        JSONAPIComponent(app, {
          include: [

...
describe('loopback json api component find methods', function () {
  ...
  describe('Headers', function () {
    it(
      'GET /models should have the JSON API Content-Type header set on collection responses',
      function (done) {
        request(app)
          .get('/posts')
          .expect(200)
    ...
    it(
      'GET /models/:id should have the JSON API Content-Type header set on individual resource responses',
      function (done) {
        request(app)
          .get('/posts/1')
          .expect(404)
  ...
  describe('Relationship objects', function () {
    ...
    it(
      'should not be present when no relationships have been defined on a collection',
      function (done) {
    ...
    it(
      'should not be present when no relationships have been defined on an individual resource',
      function (done) {
  ...
  describe('Self links', function () {
    ...
    it(
      'GET /posts/2 should produce correct resource level self links for individual resources',
      function (done) {
        request(app).get('/posts/2').expect(200).end(function (err, res) {
          expect(err).to.equal(null)
          expect(res.body.data.links.self).to.match(
      ...
      function (done) {
        request(app).get('/posts').expect(200).end(function (err, res) {
          ...
          res.body.data.forEach(function (resource) {
            expect(resource.links.self).to.match(
              new RegExp('^http://127.0.0.1.*/posts/' + index)
            )
            index++
          })
  ...
  describe('Empty responses', function () {
    it(
      'GET /models should return an empty JSON API resource object when there are no results',
      function (done) {
        request(app).get('/posts').expect(200).end(function (err, res) {
          expect(err).to.equal(null)
    ...
    it(
      'GET model/:id should return a general 404 when there are no results',
      function (done) {
        request(app).get('/posts/1').expect(404).end(done)
      }
    )
  })
  ...
  describe('Non-empty reponses', function () {
    ...
    it('GET /models/ should return a JSON API response with 1 item', function (
      done
    ) {
      request(app).get('/posts').expect(200).end(function (err, res) {
        expect(err).to.equal(null)
    ...
    it('GET /models/:id should return a correct JSON API response', function (
      done
    ) {
      request(app).get('/posts/1').expect(200).end(function (err, res) {
        expect(err).to.equal(null)
  ...
  describe('Errors', function () {
    it('GET /models/doesnt/exist should return a general 404 error', function (
      done
    ) {
      request(app).get('/posts/doesnt/exist').expect(404).end(done)
    })
  })

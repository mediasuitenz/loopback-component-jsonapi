...
describe('loopback json api hasOne relationships', function () {
  ...
  describe('Post without an author', function (done) {
    ...
    describe('GET /posts/1/author', function () {
      it('should return status code 200 OK', function (done) {
        request(app).get('/posts/1/author').expect(200).end(done)
      })

      it('should return `null` keyed by `data`', function (done) {
        request(app).get('/posts/1/author').end(function (err, res) {
    ...
    describe('GET /posts/1/relationships/author', function () {
      it('should return status code 200 OK', function (done) {
        request(app).get('/posts/1/relationships/author').expect(200).end(done)
      })
  ...
  describe('Post with an author', function (done) {
    ...
    describe(
      ...
      function () {
        it(
          'should return author relationship link in relationships object',
          function (done) {
            request(app).get('/posts').end(function (err, res) {
              ...
              expect(
                res.body.data[0].relationships.author.links.related
        ...
        it(
          ...
          function (done) {
            request(app).get('/posts').end(function (err, res) {
              expect(err).to.equal(null)
              expect(res.body.data[0].relationships.author).not.to.have.key(
                'data'
              )
              done()
            })
        ...
        it(
          ...
          function (done) {
            request(app)
              .get('/posts?filter[include]=author')
              .end(function (err, res) {
                expect(err).to.equal(null)
                expect(res.body.data[0].relationships).to.be.an('object')
                expect(res.body.data[0].relationships.author).to.be.an(
                  'object'
                )
                expect(
        ...
        it(
          'should allow specifying `include` in the url to meet JSON API spec. eg. include=author',
          function (done) {
            request(app).get('/posts?include=author').end(function (err, res) {
              expect(err).to.equal(null)
              expect(res.body.included).to.be.an('array')
              expect(res.body.included.length).to.equal(1)
      ...
      function () {
        it(
          ...
          function (done) {
            request(app).get('/posts/1').end(function (err, res) {
              ...
              expect(res.body.data.relationships.author.links.related).to.match(
                /posts\/1\/author/
              )
              done()
            })
        ...
        it(
          ...
          function (done) {
            request(app).get('/posts/1').end(function (err, res) {
              expect(err).to.equal(null)
              expect(res.body.data.relationships.author).not.to.have.key(
                'data'
              )
              done()
            })
          ...
          function (done) {
            request(app)
              ...
              .end(function (err, res) {
                ...
                expect(
                  res.body.data.relationships.author.links.related
                ).to.match(/posts\/1\/author/)
                expect(res.body.included).to.be.an('array')
                expect(res.body.included.length).to.equal(1)
                expect(res.body.included[0]).to.have.all.keys(
        ...
        it(
          'should allow specifying `include` in the url to meet JSON API spec. eg. include=author',
          function (done) {
            request(app).get('/posts/1?include=author').end(function (err, res) {
              expect(err).to.equal(null)
              expect(res.body.included).to.be.an('array')
              expect(res.body.included.length).to.equal(1)
              done()

 ...
 * @return {undefined}
 */
function JSONAPIErrorHandler (err, _req, res, _next) {
  ...
  if (err.details && err.details.messages) {
    ...
    let errorSource = err.source && typeof err.source === 'object'
      ? err.source
      : {}
  ...
  debug('Response Object:', { errors })
  res.status(statusCode).send({ errors }).end()
}

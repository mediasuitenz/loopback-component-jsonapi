...
module.exports = function (app, options) {
  ...
  app.remotes().before('**', function (ctx, next) {
    let data, serverRelations, errors

    if (utils.shouldNotApplyJsonApi(ctx, options)) {
      return next()
    }

    const regexs = [/^create$/, /^updateAttributes$/, /^patchAttributes$/]
    ...
    if (RelUtils.isRequestingIncludes(ctx)) {
      if (!ctx.res.headersSent) {
        options.debug('Set response Content-Type to `application/vnd.api+json`')
        ctx.res.set({
      ...
      let include = RelUtils.getIncludesArray(ctx.req.query)
